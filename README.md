...
The mock AI is an **ADP-led, roster-aware simulation**, not a claim about how real managers draft. It samples a normal ADP shock with standard deviation `min(10, 1.15 + 0.55 × (round - 1))` picks, so Round 1 stays tight while later selections vary more. Candidate probability is proportional to the ADP fit after that shock, adjusted for core starter needs (1 QB, 2 RB, 2 WR, 1 TE), selected W/R/T flex capacity, early K/DEF avoidance, and excess-position penalties. Large Guru-versus-market value can offset—but not erase—an oversupply penalty, preventing normal five-RB starts while retaining plausible BPA deviations. Every AI choice is unique; the 198-player source board intentionally leaves late cells as pick placeholders if a 14-team, 15-round matrix exceeds available records.

The draft room is a responsive three-pane analyzer: the left pane filters the available player pool by name, position, or roster need; the center pane keeps the live snake board and complete 15-round team matrix visible; and the right pane highlights the recommended target, rationale, alternatives, starter/flex construction, and your roster. Matrix cells show overall pick numbers until drafted, then player, position, and bye; current pick and user-team column are highlighted. Source-backed Guru bye weeks appear in player cards, queue entries, the rankings table, draft board, recommendations, and rosters. The roster pane summarizes bye-week concentration as informational context only; it never blocks a selection. A missing source bye is shown as **Bye unavailable**.
