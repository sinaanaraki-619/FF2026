...
- Set format, league size, draft position, flexes, mode, seed, and AI risk, then select **Start Live Draft** or **Start Mock Draft**. Starting locks those settings and shows the active configuration. **Restart / unlock settings** asks for confirmation, clears the current board, and makes setup editable again.
